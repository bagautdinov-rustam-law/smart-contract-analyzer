...
## 🔧 Настройка DeepSeek Thinking Mode

1. Зарегистрируйтесь и создайте ключ в [DeepSeek Console](https://platform.deepseek.com/api_keys) — Thinking Mode ключи начинаются с `sk-`.
2. Добавьте один или несколько Thinking Mode ключей в переменную окружения `VITE_DEEPSEEK_API_KEYS`, разделяя их запятой для пуллинг/ротации.
3. Убедитесь, что используете параметры Thinking Mode в вызовах API: модель `deepseek-reasoner`, включенный reasoning (`thinking`/`reasoning` режим) и разумный лимит мыслей (`max_thinking_tokens`, например 8192), чтобы не превышать квоты.
4. При необходимости ограничивайте `temperature` и `top_p` в соответствии с требованиями аналитики, чтобы Thinking Mode отвечал стабильно.
...
│   │   │   ├── use-gemini-analysis.ts       # Хук для анализа (логика DeepSeek Thinking Mode)
│   │   │   └── use-mobile.tsx               # Хук для мобильной адаптации
│   │   ├── lib/                     # Утилиты и библиотеки
│   │   │   ├── gemini.ts                    # Основная логика анализа (DeepSeek Thinking Mode)
...
- **Пул API ключей**: Поддержка нескольких DeepSeek Thinking Mode API ключей для увеличения лимитов и надежности
...
- [DeepSeek](https://platform.deepseek.com/) за предоставление Thinking Mode API для аналитики текста
...
- **Телеграм-канал**: [AI Скрепка: Связь Права и Технологий](https://t.me/+plgepYs_y0M3ODJi)
- **DeepSeek Console**: [Получить Thinking Mode ключ](https://platform.deepseek.com/api_keys)
